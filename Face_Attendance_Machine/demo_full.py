...
def faceRegiser(faceId=1, userName='default', interval=3, faceCount=3, resize_w=700, resize_h=400):
    ...
    :param faceCount: 采集数量
    :param resize_w: 缩放后宽度
    :param resize_h: 缩放后高度
    :return:
    """
    # 计数
    count = 0
        ...
        for face in face_detetion:
            ...
            if (now - show_time) < 0.5:
                frame = cv2AddChineseText(frame,
                                          "注册成功 {count}/{faceCount}".format(
                                              count=(count+1),
                                              faceCount=faceCount),
                                          (l, b+30), textColor=(255, 0, 255), textSize=40)
            ...
            if count < faceCount:
                ...
                if now - startTime > interval:
                    ...
                    frame = cv2AddChineseText(frame, "注册成功 {count}/{faceCount}".format(count=(count+1),
                                                                                       faceCount=faceCount),
                                              (l, b+30), textColor=(255, 0, 255), textSize=40)
                    show_time = time.time()
                    # 时间重置
...
def faceRecognize(detector='haar', threshold=0.5, write_video=False, resize_w=700, resize_h=400):
    ...
    while True:
        ...
        for face in face_detetion:
            ...
            # 绘制人脸点
            cv2.putText(frame, predict_name + " " + str(round(min_distance, 2)),
                        (l, b+30), cv2.FONT_ITALIC, 0.8, (0, 255, 0), 2)
...
parser.add_argument("--name", type=str, default='daichen',
                    help="人脸姓名，英文格式")                       
...
if mode == 'reg':
    faceRegiser(faceId=args.id, userName=args.name, interval=args.interval,
                faceCount=args.count, resize_w=args.w, resize_h=args.h)

if mode == 'recog':
    faceRecognize(detector=args.detector, threshold=args.threshold,
                   write_video=args.record, resize_w=args.w, resize_h=args.h)